...
def removeOutliers(data):
    try:
        ...
        for feature in data.keys():    
            ...
            #ind = np.append(ind,np.array(good_data.index))
            ind.extend(np.array(good_data.index))
        
        print ind
        outliers = set([x for x in ind if ind.count(x) > 1])
        print sorted(outliers)
        return good_data, sorted(outliers)
    except Exception as e:
        print "-----------------------------------------------------------------------"
        print("Exception in function:logScaling.py"+ str(e))
