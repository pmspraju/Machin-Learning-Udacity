...
from projectFunctions import loadData,featureRelevance,testNormality,logScaling,removeOutliers,pcaImplement,gmmCluster,dataRecovery,samples
...
#samples
indices = [4,44,220]
samples = samples(data,indices)
#samples_bar = samples.append(data.describe().loc['mean'])
samples_ext = samples.append(data.describe().loc['mean'])
samples_bar = samples_ext.append(data.describe().loc['50%'])
...
#removing outliers
outliers_features, outliers = removeOutliers(log_data)
#outliers  = [65,66,154]
